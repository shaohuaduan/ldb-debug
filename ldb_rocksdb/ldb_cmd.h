...
class LDBCommand {
 public:
  ...
  static const std::string ARG_TIMESTAMP; // const Slice* timestamp = nullptr;
  ...
  static const std::string ARG_ROW_CACHE;//std::shared_ptr<Cache> row_cache = nullptr;
  ...
  static const std::string ARG_ITERATE_LOWER_BOUND;// const Slice* iterate_lower_bound = nullptr;
  static const std::string ARG_ITERATE_UPPER_BOUND;// const Slice* iterate_upper_bound = nullptr;
  ...
  static const std::string ARG_TAILING;// bool tailing = false;
  // bool managed = removed;
  static const std::string ARG_TOTAL_ORDER_SEEK;// bool total_order_seek = true;
  static const std::string ARG_AUTO_PREFIX_MODE;// bool auto_prefix_mode = unknown;
  static const std::string ARG_PREFIX_SAME_AS_START;// bool prefix_same_as_start = false;
  static const std::string ARG_PIN_DATA;// bool pin_data = false;
  static const std::string ARG_BACKGROUND_PURGE_ON_ITERATOR_CLEANUP;// bool background_purge_on_iterator_cleanup = false;
  static const std::string ARG_IGNORE_RANGE_DELETIONS;// bool ignore_range_deletions = false;
  // std::function<bool(const TableProperties&)> table_filter;// Default: empty (every table will be scanned)
  static const std::string ARG_ITER_START_SEQNUM;// SequenceNumber iter_start_seqnum = 0;
  // const Slice* timestamp = nullptr;

  // -------------------------------------- write options --------------------------------------

  static const std::string ARG_SYNC;// bool sync = false; leveldb/hyperleveldb ONLY
  static const std::string ARG_DISABLEWAL;  // bool disableWAL = false;
  static const std::string ARG_IGNORE_MISSING_COLUMN_FAMILIES;  // bool ignore_missing_column_families = false;
  static const std::string ARG_NO_SLOWDOWN;  // bool no_slowdown = false;
  static const std::string ARG_LOW_PRI;  // bool low_pri = false;
  static const std::string ARG_MEMTABLE_INSERT_HINT_PER_BATCH;  // bool memtable_insert_hint_per_batch = false;
  // const Slice* timestamp = nullptr;
  ...
  static LDBCommand* InitCommand(const std::string command, DB* db, DBWithTTL* db_with_ttl, std::vector<const Snapshot*> snapshot, std::vector<Iterator*> iterator, WriteBatchWithIndex* batch);
  ...
  void SetDBWithTTL( DBWithTTL* db_with_ttl) { db_with_ttl_ = db_with_ttl; } 
  ...
  void SetBatch(WriteBatchWithIndex* batch){ batch_ = batch; }
 ...
 protected:
  ...
  WriteBatchWithIndex* batch_ = nullptr;
  std::string iterate_lower_bound_;
  std::string iterate_upper_bound_;
   ...
   * passed in.
   */
  bool ParseDBOption(const std::map<std::string, std::string>& options,
             const std::vector<std::string>& flags);
...
uint64_t DecodeFixed64(const char* ptr);
   ...
   * Otherwise an exception is thrown.
   */
std::string ParseKey(const Slice& internal_key);
...
class OpenDBWithTTLCommand : public LDBCommand {
 public:
  static std::string Name() { return "opendbwithttl"; }

  OpenDBWithTTLCommand(const std::vector<std::string>& params,
                const std::map<std::string, std::string>& options,
                const std::vector<std::string>& flags);
...
class OpenAsSecondaryCommand : public LDBCommand {
 public:
  static std::string Name() { return "openassecondary"; }

  OpenAsSecondaryCommand(const std::vector<std::string>& params,
                const std::map<std::string, std::string>& options,
                const std::vector<std::string>& flags);
...
class PutWithTTLCommand : public LDBCommand {
 public:
  static std::string Name() { return "putwithttl"; }

  PutWithTTLCommand(const std::vector<std::string>& params,
                const std::map<std::string, std::string>& options,
                const std::vector<std::string>& flags);
...
class WBWIPutCommand : public LDBCommand {
 public:
  static std::string Name() { return "wbwiput"; }

  WBWIPutCommand(const std::vector<std::string>& params,
                const std::map<std::string, std::string>& options,
                const std::vector<std::string>& flags);
...
class WBWIDeleteCommand : public LDBCommand {
 public:
  static std::string Name() { return "wbwidelete"; }

  WBWIDeleteCommand(const std::vector<std::string>& params,
                const std::map<std::string, std::string>& options,
                const std::vector<std::string>& flags);
...
class WBWIIteratorCommand : public LDBCommand {
 public:
  static std::string Name() { return "wbwiiterator"; }

  WBWIIteratorCommand(const std::vector<std::string>& params,
                const std::map<std::string, std::string>& options,
                const std::vector<std::string>& flags);
...
class WBWIWriteCommand : public LDBCommand {
 public:
  static std::string Name() { return "wbwiwrite"; }

  WBWIWriteCommand(const std::vector<std::string>& params,
                const std::map<std::string, std::string>& options,
                const std::vector<std::string>& flags);
...
class GetWithTTLCommand : public LDBCommand {
 public:
  static std::string Name() { return "getwithttl"; }

  GetWithTTLCommand(const std::vector<std::string>& params,
             const std::map<std::string, std::string>& options,
             const std::vector<std::string>& flags);
...
class MultiGetCommand : public LDBCommand {
 public:
  static std::string Name() { return "multiget"; }
...
class DeleteRangeWithTTLCommand : public LDBCommand {
 public:
  static std::string Name() { return "deleterangewithttl"; }

  DeleteRangeWithTTLCommand(const std::vector<std::string>& params,
             const std::map<std::string, std::string>& options,
             const std::vector<std::string>& flags);
...
class DeleteDBWithTTLCommand : public LDBCommand {
 public:
  static std::string Name() { return "deletedbwithttl"; }

  DeleteDBWithTTLCommand(const std::vector<std::string>& params,
                const std::map<std::string, std::string>& options,
                const std::vector<std::string>& flags);
...
class CloseCommand : public LDBCommand {
 public:
  static std::string Name() { return "close"; }

  CloseCommand(const std::vector<std::string>& params,
                const std::map<std::string, std::string>& options,
                const std::vector<std::string>& flags);
...
class CloseWithTTLCommand : public LDBCommand {
 public:
  static std::string Name() { return "closewithttl"; }

  CloseWithTTLCommand(const std::vector<std::string>& params,
                const std::map<std::string, std::string>& options,
                const std::vector<std::string>& flags);
...
class FlushCommand : public LDBCommand {
 public:
  static std::string Name() { return "flush"; }

  FlushCommand(const std::vector<std::string>& params,
                const std::map<std::string, std::string>& options,
                const std::vector<std::string>& flags);
...
class IteratorRefreshCommand : public LDBCommand {
 public:
  static std::string Name() { return "iteratorrefresh"; }

  IteratorRefreshCommand(const std::vector<std::string>& params,
                const std::map<std::string, std::string>& options,
                const std::vector<std::string>& flags);
