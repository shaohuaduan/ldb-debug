...
LDBCommand* LDBCommand::InitCommand(const std::string cmd, DB* db, std::vector<const Snapshot*> snapshot, std::vector<Iterator*> iterator) {
  
  std::vector<std::string> cmdSplits = StrSplit(cmd, ' ');
#ifdef DEBUG
  std::cout  << "(Ldb-Tool) Command: " << cmd << ", Command size: " << cmdSplits.size() <<std::endl;
#endif
  return InitCommand(cmdSplits, db, snapshot, iterator, SelectCommand);
}
